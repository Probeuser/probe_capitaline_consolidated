...
try : 
        ...
        # Capitaline Screener >> Consolidated >> (Rs In Crores    )
        ...
        Downloaded_File_Path = r"C:\Users\stagadmin\CAPITALINE\consolidateCapitaline\downloadExcel\Download_consolidate_20231228.xlsx"
        # Downloaded_File_Path = f'"{excel_file_path}"'
        print(Downloaded_File_Path)
        df_copy = pd.read_excel(Downloaded_File_Path, header=None)
        df = pd.read_excel(Downloaded_File_Path, header=None)
        print(df_copy)
        print(f"df_copy[0]=======", df_copy.iloc[0, 0])
        # [182 rows x 46 columns]
        print("All values in the first column:")
        for value in df_copy[0]:
         print(value)
        ...
        # insert_query = f"INSERT INTO capitaline_new_download (NatureReport,{columns_str}, scraping_date) VALUES (%s,{', '.join(['%s'] * len(column_names))}, %s)"

        # Your SQL INSERT query template with placeholders for values
        insert_query = f"INSERT INTO capitaline_download_trial ({columns_str},scraping_date) VALUES ({', '.join(['%s'] * len(column_names))},%s)"

        # Your SQL SELECT query template
        select_query = """SELECT * FROM capitaline_download_trial WHERE NatureReport = %s and  CapitalineCode = %s  and YearEnd = %s and ResultType = %s LIMIT 1"""
        # select_query = """SELECT * FROM capitaline_download_trial """

        # Your SQL UPDATE query template
        update_query = """
            UPDATE capitaline_download_trial
            SET
            NatureReport  = %s,
            CapitalineCode = %s,
            CompanyName = %s,
        ...
        for iterate in range(subtitle_index + 2, disclaimer_index):
            ...
            print("index======",iterate)
            print(iterate - subtitle_index - 1)
            # row_data = df_new.iloc[iterate - subtitle_index - 1].tolist()
            ...
            if existing_row:
                # If row exists, perform UPDATE for specific columns
                update_data = [row_data[column_names.index(column)] for column in update_columns]
                print("update_data========")
                print(update_data)
                print("update_data========2")
                print(update_data + [row_data[0], row_data[1], row_data[3], row_data[4]])
                print(update_query)
        ...
        if update_length > 0 :
            ...
            comments = ""
        # updateDataScrapeLog("Capitaline_consolidate",status,length,scraped_length,reason,comments,tradeDate) 
